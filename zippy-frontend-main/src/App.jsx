...
const BACKEND_URL = import.meta.env.VITE_BACKEND_URL || (window.location.hostname === 'localhost' ? 'http://localhost:8080' : 'https://zippy-zdgf.onrender.com');
const API_URL = `${BACKEND_URL}/api`;
const socket = io(BACKEND_URL);
const FALLBACK_IMAGE = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="200" height="200" viewBox="0 0 200 200"><rect width="200" height="200" fill="%23f3f4f6"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="%239ca3af" font-family="sans-serif" font-size="14">No Image</text></svg>';
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50/90 via-indigo-50/70 to-purple-50/80 text-gray-900 font-sans overflow-x-hidden selection:bg-blue-200 selection:text-blue-900 pb-28 md:pb-0 relative z-0">
      {/* ========================================== */}
      {editingProduct && (
        <EditProductModal 
      ...
      <style>{`
        ...
        .sunlit-glow {
          background: radial-gradient(circle at top left, rgba(255, 255, 255, 0.8) 0%, rgba(255, 255, 255, 0) 65%);
        }
      `}</style>

      {/* --- AESTHETIC LIQUID BLOBS BACKGROUND --- */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none -z-10">
        <div className="absolute -top-40 -left-40 w-[550px] h-[550px] bg-gradient-to-tr from-blue-400/35 via-cyan-300/30 to-indigo-400/30 rounded-full filter blur-[90px] opacity-70 animate-liquid-blob-1"></div>
        <div className="absolute top-1/3 -right-40 w-[600px] h-[600px] bg-gradient-to-tr from-purple-400/30 via-pink-400/25 to-blue-400/25 rounded-full filter blur-[100px] opacity-70 animate-liquid-blob-2"></div>
        <div className="absolute -bottom-40 left-1/3 w-[650px] h-[650px] bg-gradient-to-tr from-emerald-300/25 via-teal-300/20 to-blue-400/30 rounded-full filter blur-[110px] opacity-60 animate-liquid-blob-3"></div>
      </div>

      {/* --- LIQUID GLASS DESKTOP NAVBAR --- */}
      <nav className="hidden md:block liquid-glass-header px-4 md:px-8 py-3.5 sticky top-0 z-40 transition-all">
        <div className="flex justify-between items-center max-w-[1400px] mx-auto">
          <div className="flex items-center space-x-8">
            <div className="flex items-center gap-1 cursor-pointer hover:scale-105 transition-transform" onClick={() => { if((!user || user.role === 'CUSTOMER')) { setActiveCategory('All'); setView('home'); setSelectedProduct(null); } }}>
              <span className="text-4xl font-black tracking-tighter text-blue-600 drop-shadow-sm">zippy</span>
            </div>
            
            {/* Delivery Location */}
            {(!user || user.role === 'CUSTOMER') && (
              <div className="flex flex-col border-l border-white/60 pl-6 cursor-pointer group" onClick={() => setIsChangingLocation(!isChangingLocation)}>
                <span className="text-xs font-black text-gray-500 uppercase tracking-wider group-hover:text-blue-600 transition flex items-center gap-1">
                  Delivery Location <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 text-blue-600" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
                </span>
                <span className="text-[15px] font-bold text-gray-900 truncate w-56 drop-shadow-2xs">{location}</span>
              </div>
            )}
          </div>
          ...
          {(!user || user.role === 'CUSTOMER') && (
            <div className="flex-1 max-w-2xl mx-8 relative z-50">
               <div className="w-full flex items-center liquid-glass-pill rounded-2xl px-5 py-3 border border-white/80 focus-within:border-blue-500 focus-within:bg-white/90 focus-within:shadow-[0_8px_25px_rgba(37,99,235,0.2)] transition-all">
               ...
               {showSuggestions && searchSuggestions.length > 0 && (
                  <div className="absolute top-full left-0 right-0 mt-2 liquid-glass rounded-2xl shadow-2xl border border-white/90 overflow-hidden py-2 animate-fade-in-up">
                     {searchSuggestions.map(item => (
                        <div key={item._id} onClick={() => { openProduct(item); setSearchQuery(''); setShowSuggestions(false); }} className="px-5 py-3 hover:bg-blue-50/80 cursor-pointer flex items-center justify-between group transition-colors">
                           <div className="flex items-center gap-3">
                              <div className="w-10 h-10 bg-white/70 rounded-lg flex items-center justify-center p-1 border border-white/80 shadow-sm">
          ...
          <div className="flex items-center space-x-8">
            {!user ? (
              <button onClick={() => setIsAuthOpen(true)} className="font-black text-gray-700 hover:text-blue-600 transition cursor-pointer">Login</button>
            ) : (
            ...
            {(!user || user.role === 'CUSTOMER') && (
              <button onClick={() => setIsCartOpen(true)} className="flex items-center gap-2 liquid-glass-button text-white px-6 py-2.5 rounded-2xl font-black shadow-[0_8px_25px_rgba(37,99,235,0.4)] cursor-pointer">
                ...
                <span>My Cart</span>
                {cartItemCount > 0 && <span className="bg-white text-blue-600 px-2 py-0.5 rounded-full text-xs ml-1 shadow-sm font-black">{cartItemCount}</span>}
              </button>
            )}
          </div>
        </div>
      ...
      {/* --- AESTHETIC LIQUID GLASS BOTTOM NAVIGATION (MOBILE) --- */}
      {(!user || user.role === 'CUSTOMER') && (
        <div className="md:hidden fixed bottom-6 left-5 right-5 liquid-glass backdrop-blur-3xl border border-white/80 z-50 flex justify-around items-center py-3 px-3 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.18)]">
           <button onClick={() => setView('home')} className={`flex flex-col items-center gap-1 px-4 py-1 rounded-2xl transition-all ${view === 'home' ? 'text-blue-600 font-black' : 'text-gray-500 hover:text-gray-800'}`}>
           ...
           <button onClick={() => setView('categories')} className={`flex flex-col items-center gap-1 px-4 py-1 rounded-2xl transition-all ${view === 'categories' ? 'text-blue-600 font-black' : 'text-gray-500 hover:text-gray-800'}`}>
           ...
           <button onClick={() => setIsCartOpen(true)} className="relative -mt-10 liquid-glass-button text-white w-14 h-14 rounded-full flex items-center justify-center shadow-[0_12px_30px_rgba(37,99,235,0.5)] border-[3.5px] border-white cursor-pointer hover:scale-110 transition-transform z-10">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
              {cartItemCount > 0 && <span className="absolute -top-1 -right-1 bg-rose-500 text-white text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-full border-2 border-white shadow-sm">{cartItemCount}</span>}
           </button>

           <button onClick={() => setView('help')} className={`flex flex-col items-center gap-1 px-4 py-1 rounded-2xl transition-all ${view === 'help' ? 'text-blue-600 font-black' : 'text-gray-500 hover:text-gray-800'}`}>
...
const filteredProducts = activeTheme === 'All' 
  ...
  return (
    <div className="w-full pb-10">
      ...
      {/* 1. LIQUID GLASS DYNAMIC HEADER SECTION */}
      <div className="w-full max-w-[1400px] mx-auto px-2 md:px-6 pt-2 md:pt-4">
        <div className="w-full liquid-glass-hero rounded-[2rem] md:rounded-[2.5rem] p-3 md:p-6 shadow-2xl relative overflow-hidden transition-all duration-700">
          
          {/* Ambient Glow Orbs */}
          <div className="absolute -top-20 -left-20 w-80 h-80 bg-blue-400/30 rounded-full blur-3xl pointer-events-none animate-pulse"></div>
          <div className="absolute -bottom-20 -right-20 w-80 h-80 bg-purple-400/30 rounded-full blur-3xl pointer-events-none animate-pulse" style={{ animationDelay: '1.5s' }}></div>

          {/* Mobile Header Elements */}
          <div className="md:hidden px-2 pt-2 pb-3 text-white relative z-10">
             <div className="flex justify-between items-center mb-3">
                <div onClick={() => setIsChangingLocation(true)} className="cursor-pointer">
                   <h2 className="font-black text-[26px] tracking-tight leading-none mb-1 drop-shadow-md">15 mins</h2>
                   <p className="text-xs font-bold opacity-95 truncate max-w-[240px] flex items-center gap-1 drop-shadow-sm">
                     To {location} <span className="text-[10px]">▼</span>
                   </p>
                </div>
                <div onClick={() => !user ? setIsAuthOpen(true) : setView('account')} className="w-10 h-10 liquid-glass-pill rounded-full flex items-center justify-center text-xl shadow-md border border-white/60 cursor-pointer hover:bg-white/40 transition">
             ...
             <div className="relative z-50">
               <div className="liquid-glass-pill rounded-2xl px-4 py-3 flex items-center shadow-md border border-white/80 focus-within:border-blue-300 transition-all">
               ...
               {showSuggestions && searchSuggestions.length > 0 && (
                  <div className="absolute top-full left-0 right-0 mt-2 liquid-glass rounded-2xl shadow-2xl border border-white/80 overflow-hidden py-2">
                     {searchSuggestions.map(item => (
                        <div key={item._id} onClick={() => { openProduct(item); setSearchQuery(''); setShowSuggestions(false); }} className="px-4 py-3 hover:bg-white/40 border-b last:border-b-0 border-white/40 cursor-pointer flex items-center justify-between">
                           <div className="flex items-center gap-3">
                              <img src={item.imagePath.startsWith('http') ? item.imagePath : getImgSrc(item.imagePath)} className="w-8 h-8 object-contain mix-blend-multiply" alt="" onError={(e) => e.target.src='https://via.placeholder.com/50'} />
          ...
          {/* LIQUID CATEGORY NAV TABS */}
             <div className="flex overflow-x-auto hide-scroll gap-4 md:gap-6 px-2 md:px-4 pt-2 md:pt-3 pb-3 border-b border-white/30">
                {Object.values(THEMES).map((tab) => (
                   <div key={tab.label} onClick={() => setActiveTheme(tab.label)} className={`flex items-center gap-2 cursor-pointer min-w-max px-4 py-2 rounded-2xl transition-all duration-300 ${activeTheme === tab.label ? 'bg-white/35 backdrop-blur-md border border-white/80 text-white shadow-lg shadow-white/10 scale-105 font-black' : 'hover:bg-white/15 text-white/80 hover:text-white font-bold'}`}>
             ...
             <div className="px-2 md:px-4 py-4 md:py-6 flex justify-between items-center relative overflow-hidden">
                <div className="absolute right-[-20px] top-[-10px] opacity-15 text-[120px] transform rotate-12 pointer-events-none select-none">⚡</div>
                <h2 className="text-[30px] md:text-5xl font-black italic transform -skew-x-[12deg] drop-shadow-[0_4px_12px_rgba(0,0,0,0.3)] leading-none">
                   ⭐ <span className="bg-gradient-to-r from-yellow-200 via-amber-300 to-yellow-400 bg-clip-text text-transparent">ZIPPY</span> <br className="md:hidden"/> <span className="text-white">SUPER SALE</span> <span className="text-2xl md:text-4xl">🔥</span>
                </h2>
             </div>  

             {/* UNIFORM LIQUID GLASS BANNERS */}
             <div className="w-full pb-2 md:pb-4">
                <div className="flex overflow-x-auto snap-x hide-scroll gap-4 px-2 md:px-4">
                   
                   <div className="min-w-[270px] md:min-w-[320px] rounded-[1.75rem] snap-center relative liquid-glass-banner cursor-pointer group overflow-hidden h-[150px] md:h-[175px] hover:-translate-y-1 transition-all border border-white/50 shadow-xl">
                      <img src="https://images.unsplash.com/photo-1550547660-d9450f859349?auto=format&fit=crop&w=600&q=80" className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 z-0 opacity-75" alt="Snacks"/>
                      <div className="absolute inset-0 bg-gradient-to-r from-gray-950/90 via-gray-900/60 to-transparent z-10"></div>
                      <div className="relative z-20 p-5 flex flex-col justify-center h-full w-[85%]">
                        <h3 className="text-white font-black text-xl md:text-2xl leading-tight mb-1 drop-shadow-md">Midnight Munchies</h3>
                        <p className="text-rose-300 font-bold text-xs tracking-wide">DELIVERED TILL 3 AM</p>
                      </div>
                   </div>

                   <div className="min-w-[270px] md:min-w-[320px] rounded-[1.75rem] snap-center relative liquid-glass-banner cursor-pointer group overflow-hidden h-[150px] md:h-[175px] hover:-translate-y-1 transition-all border border-white/50 shadow-xl">
                      <img src="https://images.unsplash.com/photo-1542838132-92c53300491e?auto=format&fit=crop&w=600&q=80" className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 z-0 opacity-75" />
                      <div className="absolute inset-0 bg-gradient-to-r from-emerald-950/90 via-emerald-900/60 to-transparent z-10"></div>
                      <div className="relative z-20 p-5 flex flex-col justify-center h-full w-[85%]">
                        <h3 className="text-white font-black text-xl md:text-2xl leading-tight mb-1 drop-shadow-md">Organic Greens</h3>
                        <p className="text-emerald-300 font-bold text-xs tracking-wide">DIRECT FROM FARMS</p>
                      </div>
                   </div>

                   <div className="min-w-[270px] md:min-w-[320px] rounded-[1.75rem] snap-center relative liquid-glass-banner cursor-pointer group overflow-hidden h-[150px] md:h-[175px] hover:-translate-y-1 transition-all border border-white/50 shadow-xl">
                      <img src="https://images.unsplash.com/photo-1604908176997-125f25cc6f3d?auto=format&fit=crop&w=600&q=80" className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 z-0 opacity-75" />
                      <div className="absolute inset-0 bg-gradient-to-r from-red-950/90 via-red-900/60 to-transparent z-10"></div>
                      <div className="relative z-20 p-5 flex flex-col justify-center h-full w-[85%]">
                        <h3 className="text-white font-black text-xl md:text-2xl leading-tight mb-1 drop-shadow-md">Protein Power</h3>
                        <p className="text-red-300 font-bold text-xs tracking-wide">FRESH CHICKEN & EGGS</p>
                      </div>
                   </div>

                   <div className="min-w-[270px] md:min-w-[320px] rounded-[1.75rem] snap-center relative liquid-glass-banner cursor-pointer group overflow-hidden h-[150px] md:h-[175px] hover:-translate-y-1 transition-all border border-white/50 shadow-xl">
                      <img src="https://images.unsplash.com/photo-1513558161293-cdaf765ed2fd?auto=format&fit=crop&w=600&q=80" className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 z-0 opacity-75" />
                      ...
                      <div className="relative z-20 p-5 flex flex-col justify-center h-full w-[85%]">
                        ...
                        <p className="text-cyan-300 font-bold text-xs tracking-wide">ICE & MIXERS</p>
                      </div>
                   </div>

                   <div className="min-w-[270px] md:min-w-[320px] rounded-[1.75rem] snap-center relative liquid-glass-banner cursor-pointer group overflow-hidden h-[150px] md:h-[175px] hover:-translate-y-1 transition-all border border-white/50 shadow-xl">
                      <img src="https://images.unsplash.com/photo-1621607512214-68297480165e?auto=format&fit=crop&w=600&q=80" className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 z-0 opacity-75" />
                      <div className="absolute inset-0 bg-gradient-to-r from-fuchsia-950/90 via-fuchsia-900/60 to-transparent z-10"></div>
                      <div className="relative z-20 p-5 flex flex-col justify-center h-full w-[85%]">
                        <h3 className="text-white font-black text-xl md:text-2xl leading-tight mb-1 drop-shadow-md">Grooming Kit</h3>
                        <p className="text-fuchsia-300 font-bold text-xs tracking-wide">MENS & WOMENS</p>
                      </div>
                   </div>

                   <div className="min-w-[270px] md:min-w-[320px] rounded-[1.75rem] snap-center relative liquid-glass-banner cursor-pointer group overflow-hidden h-[150px] md:h-[175px] hover:-translate-y-1 transition-all border border-white/50 shadow-xl">
                      <img src="https://images.unsplash.com/photo-1583337130417-3346a1be7dee?auto=format&fit=crop&w=600&q=80" className="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 z-0 opacity-75" />
                      <div className="absolute inset-0 bg-gradient-to-r from-amber-950/90 via-amber-900/60 to-transparent z-10"></div>
                      <div className="relative z-20 p-5 flex flex-col justify-center h-full w-[85%]">
                        <h3 className="text-white font-black text-xl md:text-2xl leading-tight mb-1 drop-shadow-md">Pet Supplies</h3>
                        <p className="text-amber-300 font-bold text-xs tracking-wide">FOOD & TOYS</p>
                      </div>
                   </div>
                </div>
             </div>
  ...
  return (
    <div key={p.id || p._id} onClick={() => openProduct(p)} className={`liquid-glass-card gloss-sweep-overlay p-4 rounded-[2rem] transition-all duration-500 cursor-pointer group flex flex-col h-full relative overflow-hidden border border-white/80 hover:-translate-y-2 hover:rotate-1 ${ct.bg} ${ct.border} ${ct.glow}`}>
      
      {/* Sunlit Liquid Glow */}
      <div className="absolute inset-0 sunlit-glow pointer-events-none opacity-50 group-hover:opacity-100 transition-opacity duration-500"></div>
      
      {/* Liquid Glass Badge */}
      <div className={`absolute top-0 left-0 text-white text-[9px] font-black px-3.5 py-1.5 rounded-br-2xl rounded-tl-[1.75rem] shadow-md z-10 uppercase tracking-widest liquid-glass-pill ${ct.badge}`}>⚡ Bestseller</div>
      
      {/* Product Image Container */}
      <div className={`h-28 md:h-36 w-full mb-3 rounded-2xl overflow-hidden flex items-center justify-center p-2 relative bg-gradient-to-b from-white/80 to-blue-50/40 backdrop-blur-md border border-white/70 transition-colors duration-500 mt-2 ${ct.imgBg} to-transparent shadow-inner`}>
         <img src={getImgSrc(p.imagePath)} alt={p.title} className="max-h-full max-w-full object-contain mix-blend-multiply img-float-hover group-hover:scale-110 drop-shadow-md" onError={(e) => e.target.src=FALLBACK_IMAGE} />
      </div>
      
      <div className="flex flex-col flex-1 justify-between relative z-10">
        <div>
          <div className="flex items-center gap-1.5 mb-1">
            <span className="text-[10px] bg-green-100/90 backdrop-blur-md text-green-800 px-2.5 py-0.5 rounded-full font-black border border-green-200/80 shadow-2xs">★ 4.8</span>
            <span className="text-[9px] text-gray-500 font-bold flex items-center gap-1"><span className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-pulse"></span> 10 Mins</span>
          </div>
        ...
        <div className="mt-3 pt-3 border-t border-dashed border-gray-200/80 flex items-end justify-between relative">
          <div className="flex flex-col">
            <div className="flex items-center gap-1.5">
              <span className="text-sm md:text-lg font-black text-gray-900 drop-shadow-2xs">₹{(p.price)}</span>
              <span className="text-[10px] text-gray-400 line-through font-bold">₹{(p.price * 1.15).toFixed(0)}</span>
            </div>
          </div>
          
          {/* LIQUID GLASS QTY BUTTON */}
          {(() => {
            const cartItem = cart.find(c => (c._id || c.id) === (p._id || p.id));
            
            if (cartItem && cartItem.quantity > 0) {
              return (
                <div onClick={(e) => e.stopPropagation()} className="absolute -right-2 -bottom-2 h-10 liquid-glass-pill rounded-2xl flex items-center justify-between shadow-md overflow-hidden w-[5.8rem] z-20 border border-white/90">
                   <button onClick={() => removeFromCart(p._id || p.id)} className="w-8 h-full flex items-center justify-center text-xl font-bold text-gray-800 hover:bg-white/60 active:scale-90 transition cursor-pointer">
                     −
                   </button>
                   <span className="flex-1 text-center text-xs font-black text-gray-900">
                     {cartItem.quantity}
                   </span>
                   <button onClick={() => addToCart(p)} className={`w-8 h-full flex items-center justify-center text-xl font-bold transition active:scale-90 cursor-pointer ${ct.btn.split(' ')[0]} hover:bg-white/60`}>
                     +
                   </button>
                </div>
              );
            ...
            return (
              <button onClick={(e) => { e.stopPropagation(); addToCart(p); }} className={`absolute -right-2 -bottom-2 w-10 h-10 liquid-glass-button text-white rounded-2xl flex items-center justify-center text-2xl font-bold transition-all shadow-lg active:scale-90 cursor-pointer z-20 ${ct.btn}`}>
                 +
              </button>
            );
          })()}
    ...
    <div className="max-w-6xl mx-auto pt-8 pb-32 md:pb-20 px-4 md:px-8 animate-fade-in-up relative z-10">
      ...
      <div className="grid md:grid-cols-2 gap-8 lg:gap-20">
        <div className="flex flex-col relative">
          ...
          <img src={getImgSrc(product.imagePath)} alt={product.title} className="max-h-full max-w-full object-contain mix-blend-multiply hover:scale-105 transition-transform duration-500" onError={(e) => e.target.src=FALLBACK_IMAGE} />  
          </div>
...
function EditProductModal({ product, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/products/edit/${product._id || product.id}`, {
        method: 'PUT',
        body: data
      });
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-md z-50 flex justify-center items-center p-4">
      <div className="glass-refraction p-8 rounded-[2rem] w-full max-w-[440px] shadow-2xl animate-pop-elastic gloss-sweep-overlay border border-white/80">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <input type="text" name="title" value={formData.title} onChange={handleChange} required placeholder="Product Title" className="p-3.5 bg-white/80 backdrop-blur-sm border border-gray-200/80 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500/30 text-sm font-bold text-gray-900 shadow-inner" />
          <input type="number" name="price" value={formData.price} onChange={handleChange} required placeholder="Price" className="p-3.5 bg-white/80 backdrop-blur-sm border border-gray-200/80 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500/30 text-sm font-bold text-gray-900 shadow-inner" />
          <select 
            name="category" 
            value={formData.category} 
            onChange={handleChange} 
            required 
            className="p-3.5 w-full bg-white/80 backdrop-blur-sm border border-gray-200/80 rounded-2xl text-sm font-bold text-gray-800 outline-none cursor-pointer focus:ring-2 focus:ring-blue-500/30 shadow-inner"
          >
          ...
          <label className="text-xs font-black text-gray-600 uppercase tracking-wider mt-1">Update Image (Optional):</label>
          <input 
            type="file" 
            onChange={handleFileChange} 
            className="p-2 w-full border border-gray-200 bg-white/60 rounded-2xl cursor-pointer file:cursor-pointer file:bg-blue-50 file:text-blue-700 file:border-0 file:px-4 file:py-2 file:rounded-xl file:font-bold hover:file:bg-blue-100 transition-all text-xs font-bold" 
          />
          
          <div className="flex gap-4 mt-6">
            <button type="submit" className="clay-btn text-white p-3.5 flex-1 rounded-2xl font-black text-sm cursor-pointer transition-transform active:scale-95 shadow-lg">
              Save Changes
            </button>
            <button type="button" onClick={onClose} className="clay-btn-white text-rose-600 p-3.5 flex-1 rounded-2xl font-black text-sm cursor-pointer transition-transform active:scale-95 border border-rose-100 shadow-sm">
              Cancel
            </button>
          </div>
        </form>
            ...
            {products.map(p => (
              <div key={p._id} className="bg-white p-4 rounded-[1.5rem] border border-gray-100 shadow-sm flex flex-col justify-between hover:shadow-md transition-all">
                <div>
                  ...
                  <img src={getImgSrc(p.imagePath)} alt={p.title} className="max-h-full max-w-full object-contain mix-blend-multiply" onError={(e) => e.target.src=FALLBACK_IMAGE} /> 
                  </div>
